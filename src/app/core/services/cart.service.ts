...
export class CartService {
  ...
  public async addItemToCart(product: Product) {
    const existing = this.items().find(i => i.id === product.id);
    if (existing) {
      const updated = { ...existing, quantity: existing.quantity + 1 };
      this.cartrepo.upsert(updated).subscribe(() => {
        this.items.update(list => list.map(i => (i.id === updated.id ? updated : i)));
      });
    } else {
      const newItem: CartItemEntity = { ...product, quantity: 1 };
      this.cartrepo.upsert(newItem).subscribe(() => {
  ...
  public remove(id: string): void {
    this.cartrepo.delete(id).subscribe(() => {
